...
class Ai_Subtitle(_PluginBase):
    # 插件名称
    plugin_name = "AI字幕翻译中文"
    # 插件描述
    plugin_desc = "使用whisper自动生成视频文件字幕,使用大模型翻译字幕成中文。"
    # 插件图标
    plugin_icon = "aisubtitle.png"
    # 主题色
    plugin_color = "#2C4F7E"
    # 插件版本
    plugin_version = "1.0"
    # 插件作者
    plugin_author = "B-I-T-Art"
    # 作者主页
    author_url = "https://github.com/B-I-T-Art"
    # 插件配置项ID前缀
    plugin_config_prefix = "ai_subtitle"
    # 加载顺序
    plugin_order = 14
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config=None):
        ...
        if version == "v2":
            self.setup_v2(config)
        else:
            self.setup_v1(config)
    
    def setup_v2(self, config):
        """V2版本初始化逻辑"""
